...
3609. Minimum Moves to Reach Target in Grid
    ...
    python ./src/app/2025/contests/457/3609.py
...
class Solution:
    def minMoves(self, sx: int, sy: int, tx: int, ty: int) -> int:
        ...
        mins = 0
        while (tx,ty)!=(sx,sy):
            if tx<sx or ty<sy or max(tx,ty)==0: return -1

            if tx>ty:
                hlf = tx//2
                if hlf<<1 == tx and hlf >= ty:# current state had came by adding 'x' with 'x'
                    tx = hlf
                elif tx-ty<=ty and ty>0: # current state had came by adding 'x' with 'y'
                    tx = tx-ty
                else: return -1
            elif tx<ty:
                hlf = ty//2
                if hlf<<1 == ty and hlf >= tx: # curr state had came by adding 'y' with 'y'
                    ty = hlf
                elif ty-tx <= tx and tx>0: # state came by adding 'y' with 'x'
                    ty = ty-tx
                else: return -1
            else:
                if sx>sy:
...
if __name__ == '__main__':
    tcs = []
    add = lambda sx,sy,tx,ty: tcs.append([sx,sy,tx,ty])
    add(sx = 1, sy = 2, tx = 5, ty = 4)
    add(sx = 0, sy = 1, tx = 2, ty = 3)
    add(sx = 1, sy = 1, tx = 2, ty = 2)
    add(
        sx =0,sy =1,tx =1000000000,ty =999999999
    )

    sol = Solution()
    for [sx,sy,tx,ty] in tcs:
        print(f'sx={sx} sy={sy} tx={tx} ty={ty} minMoves={sol.minMoves(sx,sy,tx,ty)}')